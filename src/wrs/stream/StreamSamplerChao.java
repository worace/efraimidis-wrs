...
class WeightedItemComparator implements Comparator<WeightedItem> {
	public int compare(WeightedItem item1, WeightedItem item2) {
		return WeightedItem.compare(item1, item2);
	}
}

public class StreamSamplerChao extends StreamSampler {
	 ...
	 * @param parSeed
	 */
	public StreamSamplerChao(int parSampleSize, long parSeed) {
			...
			if (!newItemInSample) {
				...
				if (itemsB.isEmpty()) {
					...
					if ((numOfItemsInA + numOfItemsInFeasSample) != sampleSize) {
						System.err.println("A-Chao: Total items are "
